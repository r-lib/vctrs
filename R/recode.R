...
#'
#'   - If `multiple_from = FALSE`, `from` must be a single vector of any size,
#'     which will be [cast][vctrs::theory-faq-coercion] to the type of `x`.
#'
#'   - If `multiple_from = TRUE`, `from` must be a list of vectors of any size,
...
#'
#'   - If `multiple_from = FALSE`, `to` must be a single vector of size 1 or the
#'     same size as `from`.
#'
#'   - If `multiple_to = TRUE`, `to` must be a list of vectors. The list itself
#'     must be size 1 or the same size as `from`. Each individual vector in the
#'     list must be size 1 or the same size as `x`.
#'
...
#'
#' @param unmatched Handling of locations in `x` unmatched by `from`.
#'   One of:
#'
...
#'
#' @param multiple_from,multiple_to Boolean values determining how `from` and
#'   `to` should be interpreted. See their parameter descriptions for more
#'   details.
#'
...
#'   type of `to` and `default`, unless overriden by `ptype`. The names of the
...
#'   # `vec_replace_values()`, `multiple_from`, which allows you to provide a
#'   # list of `from` vectors that each match multiple `from` values to a single
#'   # `to` value. `tribble()` can help you create these!
#'   standardize <- tibble::tribble(
#'     ~from, ~to,
...
vec_replace_values <- function(
  ...
  # `vec_recode_values()` creates a new vector and names come from any of
  # `to` or `default`, but `vec_replace_values()` updates an existing
