...
#' @inheritParams vec_recode_values
#'
#' @param x A vector.
#'
...
#'
#'   Where a vector in `cases` is `TRUE`, the corresponding value in `values`
...
#'   computed as the size of the first element of `cases`. This is only useful
#'   when you need a known output size, even when `cases` is an empty list.
#'
#' @returns
#' A vector.
#'
...
# TODO!: Export or remove. If we export, consider optimizing the atomic vector
# case. We shouldn't have to do any allocations beyond the output itself, note
# how fast `data.table::fifelse()` is because of this.
