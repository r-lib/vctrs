...
struct vctrs_compact_rep_int_info {
  int value;
  R_xlen_t size;
};
...
// [[ register() ]]
SEXP vctrs_new_vctrs_compact_rep_int(SEXP value, SEXP size) {
  VCTRS_NEW_VCTRS_COMPACT_REP(value, size, int, INTEGER_RO, int);
}
...
static SEXP vctrs_compact_rep_int_unserialize(SEXP cls, SEXP state) {
  VCTRS_COMPACT_REP_UNSERIALIZE(cls, state, int, INT, int);
}

static SEXP vctrs_compact_rep_int_duplicate(SEXP x, Rboolean deep) {
  VCTRS_COMPACT_REP_DUPLICATE(x, deep, int);
}
...
static Rboolean vctrs_compact_rep_int_inspect(SEXP x,
  ...
  Rprintf("vctrs_compact_rep_int (value: %i, size: %td, state: %s)", value, size, state);
  Rprintf("\n");
...
static SEXP vctrs_compact_rep_int_extract_subset(SEXP x, SEXP indx, SEXP call) {
  ...
  const int value = VCTRS_COMPACT_REP_INT_VALUE(info);
  const R_xlen_t size = VCTRS_COMPACT_REP_INT_SIZE(info);
  ...
  switch (TYPEOF(indx)) {
  case INTSXP: COMPACT_REP_INT_EXTRACT_SUBSET_LOOP(int, INTEGER_RO, true); break;
  case REALSXP: COMPACT_REP_INT_EXTRACT_SUBSET_LOOP(double, REAL_RO, R_FINITE(loc)); break;
  }
...
#undef COMPACT_REP_INT_EXTRACT_SUBSET_LOOP
