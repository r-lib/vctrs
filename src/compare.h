...
static inline
int nil_compare_na_smallest(r_obj* x, r_obj* y) {
  r_stop_internal("nil_compare_na_smallest", "Can't compare NULL values.");
}
static inline
int lgl_compare_na_smallest(int x, int y) {
  return int_compare_scalar(x, y);
}
static inline
int int_compare_na_smallest(int x, int y) {
  return int_compare_scalar(x, y);
}
static inline
int dbl_compare_na_smallest(double x, double y, bool nan_distinct) {
  enum vctrs_dbl_class x_class = dbl_classify(x);
  enum vctrs_dbl_class y_class = dbl_classify(y);
  ...
  r_stop_unreached("dbl_compare_na_smallest");
}
static inline
int cpl_compare_na_smallest(Rcomplex x, Rcomplex y) {
  r_stop_internal("cpl_compare_na_smallest", "Can't compare complex types.");
}
static inline
int chr_compare_na_smallest(r_obj* x, r_obj* y) {
  if (chr_equal_na_equal(x, y)) {
    return 0;
  } else if (chr_is_missing(x)) {
    return -1;
...
static inline
int raw_compare_na_smallest(Rbyte x, Rbyte y) {
  r_stop_internal("raw_compare_na_smallest", "Can't compare raw types.");
}
static inline
int list_compare_na_smallest(r_obj* x, r_obj* y) {
  r_stop_internal("list_compare_na_smallest", "Can't compare list types.");
}


static inline
int dbl_compare_na_smallest_nan_distinct(double x, double y) {
  return dbl_compare_na_smallest(x, y, true);
}
static inline
int dbl_compare_na_smallest_nan_indistinct(double x, double y) {
  return dbl_compare_na_smallest(x, y, false);
}

// -----------------------------------------------------------------------------

#define P_COMPARE_NA_SMALLEST(CTYPE, COMPARE_NA_SMALLEST) do {                    \
  return COMPARE_NA_SMALLEST(((CTYPE const*) p_x)[i], ((CTYPE const*) p_y)[j]);   \
} while (0)

#define P_COMPARE_NA_SMALLEST_NAN_DISTINCT(CTYPE, COMPARE_NA_SMALLEST) do {                     \
  return COMPARE_NA_SMALLEST(((CTYPE const*) p_x)[i], ((CTYPE const*) p_y)[j], nan_distinct);   \
} while (0)


static inline
int p_nil_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(r_obj*, nil_compare_na_smallest);
}
static inline
int p_lgl_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(int, lgl_compare_na_smallest);
}
static inline
int p_int_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(int, int_compare_na_smallest);
}
static inline
int p_dbl_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j, bool nan_distinct) {
  P_COMPARE_NA_SMALLEST_NAN_DISTINCT(double, dbl_compare_na_smallest);
}
static inline
int p_cpl_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(Rcomplex, cpl_compare_na_smallest);
}
static inline
int p_chr_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(r_obj*, chr_compare_na_smallest);
}
static inline
int p_raw_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(Rbyte, raw_compare_na_smallest);
}
static inline
int p_list_compare_na_smallest(const void* p_x, r_ssize i, const void* p_y, r_ssize j) {
  P_COMPARE_NA_SMALLEST(r_obj*, list_compare_na_smallest);
}

#undef P_COMPARE_NA_SMALLEST
#undef P_COMPARE_NA_SMALLEST_NAN_DISTINCT

static inline
int p_compare_na_smallest(const void* p_x,
                          r_ssize i,
                          const void* p_y,
                          r_ssize j,
                          const enum vctrs_type type,
                          bool nan_distinct) {
  switch (type) {
  case vctrs_type_null: return p_nil_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_logical: return p_lgl_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_integer: return p_int_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_double: return p_dbl_compare_na_smallest(p_x, i, p_y, j, nan_distinct);
  case vctrs_type_complex: return p_cpl_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_character: return p_chr_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_raw: return p_raw_compare_na_smallest(p_x, i, p_y, j);
  case vctrs_type_list: return p_list_compare_na_smallest(p_x, i, p_y, j);
  default: stop_unimplemented_vctrs_type("p_compare_na_smallest", type);
  }
}
