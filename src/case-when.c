  ...
  for (r_ssize i = 0; i < values_size; ++i) {
    ...
    if (value_size != 1) {
      if (value_size == out_size) {
        ...
        vec_check_size(value, out_size, p_value_arg, call);
      }
    }
  }

  // TODO: We really want `zap_outer()` names, where the outer names are used for errors
  // but don't make it into the final result, and we want that to work even with fallback
  // types. This is going to be horribly slow for absolutely no reason.
  r_obj* name_spec = KEEP(r_str("{inner}"));
