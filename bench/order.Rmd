...
Investigation of `vec_order_radix()` performance compared with `base::order()` using various data types and distributions of data (total size, number of groups, etc).
...
df <- bench::press(
  ...
  n_groups = n_groups,
  {
    ...
    bench::mark(vec_order_radix(x), base_order(x), iterations = 50)
  }
)
```
...
df <- bench::press(
  ...
  {
    x <- sample(n_groups, size, replace = TRUE)
    bench::mark(vec_order_radix(x), base_order(x), iterations = 10)
  }
)
```
...
df <- bench::press(
  ...
  {
    x <- sample(n_groups, size, replace = TRUE)
    bench::mark(vec_order_radix(x), base_order(x), iterations = 20)
  }
)
```
...
I imagine the increase in gc's for large sizes comes from the fact that `vec_order_radix()` uses `Rf_allocVector()` to generate its working memory, and `base_order()` uses `malloc()`, which won't trigger a gc.
...
bench::mark(vec_order_radix(df), base_order(df), iterations = 10)
```
