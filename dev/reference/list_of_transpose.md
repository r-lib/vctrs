...
- The *list size*, from
  [`vec_size(x)`](https://vctrs.r-lib.org/dev/reference/vec_size.md).

- The *element size*, from
  [`list_of_size(x)`](https://vctrs.r-lib.org/dev/reference/list-of-attributes.md).

- The *element type*, from
  [`list_of_ptype(x)`](https://vctrs.r-lib.org/dev/reference/list-of-attributes.md).

Because all three of these are required, this function only works on
fully specified
[`list_of()`](https://vctrs.r-lib.org/dev/reference/list_of.md)s, with
both `size` and `ptype` specified.
...
- x:
  ...
  A [list_of](https://vctrs.r-lib.org/dev/reference/list_of.md) with
  both `size` and `ptype` specified.
