...
- logical, numeric, character:
  ...
  [cast](https://vctrs.r-lib.org/dev/reference/vec_cast.md) to the
  relevant base type, e.g. factors are coerced to character. `NULL` is
  treated as an empty integer vector, and is thus coercible depending on
  the setting of `numeric`. Symbols are treated as character vectors and
