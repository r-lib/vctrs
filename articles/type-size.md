...
and size
([`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md)). This
vignette discusses the motivation for why these alternatives are
...
(e.g. [`new_date()`](https://vctrs.r-lib.org/reference/new_date.md),
[`new_datetime()`](https://vctrs.r-lib.org/reference/new_date.md), and
[`new_duration()`](https://vctrs.r-lib.org/reference/new_date.md)) where
the equivalents in base R are missing.

### Base prototypes

[`vec_ptype()`](https://vctrs.r-lib.org/reference/vec_ptype.md) creates
a prototype from an existing object. However, many base vectors have
uninformative printing methods for 0-length subsets, so vctrs also
provides
[`vec_ptype_show()`](https://vctrs.r-lib.org/reference/vec_ptype.md),
...
Like with
[`vec_ptype()`](https://vctrs.r-lib.org/reference/vec_ptype.md), the
...
[`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md) was
...
We define [`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md)
as follows:
...
Given [`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md), we
can give a precise definition of a data frame: a data frame is a list of
vectors where every vector has the same size. This has the desirable
property of trivially supporting matrix and data frame columns.

### Slicing

[`vec_slice()`](https://vctrs.r-lib.org/reference/vec_slice.md) is to
[`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md) as `[` is
to [`length()`](https://rdrr.io/r/base/length.html); i.e., it allows you
to select observations regardless of the dimensionality of the
underlying object. `vec_slice(x, i)` is equivalent to:
...
in [`vec_size_common()`](https://vctrs.r-lib.org/reference/vec_size.md),
