...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) is both type-
and size-stable because it possesses the following invariants:
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) obeys similar
rules:
...
if the inputs are not vectors or not automatically coercible:
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) returns a factor
taking the union of the levels. This behaviour is motivated by
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) preserves time
zones:
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) uses a standard
set of rules to avoid this problem. When you mix dates and date-times,
vctrs returns a date-time and converts dates to date-times at midnight
(in the timezone of the date-time).
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) takes a
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) is size-stable,
which implies it will row-bind data frames:
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) will “broadcast”
a vector to match the dimensions of a matrix:
...
[`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) is reasonably
simple. We first figure out the properties of the output, i.e. the
common type and total size, and then allocate it with
[`vec_init()`](https://vctrs.r-lib.org/reference/vec_init.md), and then
insert each input into the correct place in the output.
...
(The real [`vec_c()`](https://vctrs.r-lib.org/reference/vec_c.md) is a
bit more complicated in order to handle inner and outer names).
...
#> [1] NA           "2026-01-23" "2026-01-23" "2026-01-16" "2026-01-16"
```

By using [`vec_size()`](https://vctrs.r-lib.org/reference/vec_size.md)
and [`vec_slice()`](https://vctrs.r-lib.org/reference/vec_slice.md),
