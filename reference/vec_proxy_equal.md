...
vectors). For [vctr](https://vctrs.r-lib.org/reference/new_vctr.md)s,
this determines the behaviour of `==` and `!=` (via
...
[`vec_proxy()`](https://vctrs.r-lib.org/reference/vec_proxy.md), as the
default underlying vector data should be equal-able in most cases. If
your class is not equal-able, provide a `vec_proxy_equal()` method that
throws an error.
...
- [`vec_proxy()`](https://vctrs.r-lib.org/reference/vec_proxy.md) called
  by default
