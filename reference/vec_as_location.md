...
- `vec_as_location()` accepts integer, character, or logical vectors of
  ...
  subsetting with `[` or
  [`vec_slice()`](https://vctrs.r-lib.org/reference/vec_slice.md). It
  might be a different size than the input because negative selections
  are transformed to positive ones and logical vectors are transformed
