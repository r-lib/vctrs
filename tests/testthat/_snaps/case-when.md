...
# number of `cases` and `values` must be the same
    ...
    Condition
      Error in `vec_case_when()`:
      ! The number of supplied cases (1) must equal the number of supplied values (0).
...
---
    ...
    Condition
      Error in `vec_case_when()`:
      ! The number of supplied cases (2) must equal the number of supplied values (1).
...
# proof that `ptype` finalization is important
    ...
    Condition
      Error in `vec_case_when()`:
      ! Can't convert `values[[1]]` <logical> to <vctrs_unspecified>.
