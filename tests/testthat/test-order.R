...
# ------------------------------------------------------------------------------
# `vec_order_radix()` - Pre-existing tests (before switch to radix ordering)
...
test_that("can order zero column data frames (#356, #1499)", {
  df <- data_frame()
  expect_identical(vec_order_radix(df), integer())

  df <- data_frame(.size = 5L)
  expect_identical(vec_order_radix(df), 1:5)

  df <- data_frame(.size = 5L)
  expect_identical(vec_order_radix(df, direction = "desc"), 1:5)
})
...
test_that("can order zero row data frames (#356, #1499)", {
  ...
  df <- data.frame(x = numeric(), y = integer())
  expect_identical(vec_order_radix(df, direction = "desc"), integer())
})

test_that("classed proxies do not affect performance (tidyverse/dplyr#5423)", {
  skip_on_cran()
  x <- glue::glue("{1:10000}")
  expect_time_lt(vec_order_radix(x), 0.2)
})
...
test_that("can order data frames that don't allow removing the column names (#1298)", {
  ...
  expect_silent(expect_identical(vec_order_radix(df), 1L))
})
