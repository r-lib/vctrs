...
test_that("can match with df-cols of varying types", {
  ...
  df1 <- data_frame(
    x = data_frame(x = complex(real = c(1, 2), imaginary = c(2, 1)), y = y)
  )
  df2 <- data_frame(
    x = data_frame(x = complex(real = c(2, 3), imaginary = c(1, 1)), y = y)
  )
...
test_that("rcrd type matching works with rcrd-cols", {
  x <- data_frame(
    a = c(1L, 1L),
    b = new_rcrd(list(x = c(1L, 3L), y = c(1L, 4L)))
  )
  y <- data_frame(
    a = c(1L, 1L),
    b = new_rcrd(list(x = c(1L, 2L), y = c(1L, 5L)))
  )
...
test_that("Works with classed data frame columns", {
  ...
  y_col <- new_data_frame(
    list(a = c(1L, 1L, 1L), b = c(2, 4, 2)),
    class = "foo"
  )
...
test_that("`incomplete` can error informatively", {
  expect_snapshot({
    ...
    (expect_error(vec_locate_matches(
      NA,
      1,
      incomplete = "error",
      needles_arg = "foo"
...
test_that("`incomplete` error is classed", {
  expect_error(
    vec_locate_matches(NA, 1, incomplete = "error"),
...
test_that("`condition` is validated", {
  expect_error(
    vec_locate_matches(1, 2, condition = 1),
    "`condition` must be a character vector"
  )
  expect_error(
    vec_locate_matches(1, 2, condition = "x"),
    'must only contain "==", ">", ">=", "<", or "<="'
  )
  expect_error(
    vec_locate_matches(1, 2, condition = c("==", "==")),
    "must be length 1, or the same length as the number of columns of the input"
  )
})
...
test_that("`multiple` can warn informatively", {
  expect_snapshot({
    ...
    (expect_warning(vec_locate_matches(
      1L,
      c(1L, 1L),
      multiple = "warning",
      needles_arg = "foo"
...
test_that("`relationship` doesn't error errneously on the last observation", {
  expect_error(
    res <- vec_locate_matches(1:2, 1:2, relationship = "many-to-one"),
    NA
  )
  expect_identical(res$needles, 1:2)
  expect_identical(res$haystack, 1:2)
...
test_that("`relationship` is validated", {
  expect_snapshot({
    ...
    (expect_error(vec_locate_matches(
      1,
      2,
      relationship = c("one-to-one", "one-to-many")
    )))
...
test_that("`filter` is validated", {
  ...
  expect_error(
    vec_locate_matches(1, 2, filter = "x"),
    'one of "none", "min", or "max"'
  )
  expect_error(
    vec_locate_matches(1, 2, filter = c("min", "max")),
