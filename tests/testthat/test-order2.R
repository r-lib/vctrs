...
test_that("`NA` order defaults to last", {
  ...
  expect_identical(int_radix_order(x), c(1L, 3L, 2L))
})

test_that("`NA` order can be first", {
  x <- c(1L, NA_integer_, 3L)
  expect_identical(int_radix_order(x, na_last = FALSE), c(2L, 1L, 3L))
})

test_that("`decreasing` can be set to `TRUE`", {
  x <- c(1L, .Machine$integer.max, 3L)
  expect_identical(int_radix_order(x, decreasing = TRUE), c(2L, 3L, 1L))
})
...
test_that("`decreasing` doesn't interact with `na_last`", {
  ...
  expect_identical(
    x[int_radix_order(x, na_last = TRUE, decreasing = FALSE)],
    c(1L, 2L, 3L, NA_integer_)
  )
  expect_identical(
    x[int_radix_order(x, na_last = FALSE, decreasing = FALSE)],
    c(NA_integer_, 1L, 2L, 3L)
  )
  expect_identical(
    x[int_radix_order(x, na_last = TRUE, decreasing = TRUE)],
    c(3L, 2L, 1L, NA_integer_)
  )
  expect_identical(
    x[int_radix_order(x, na_last = FALSE, decreasing = TRUE)],
