...
Most of the common type determination should be performed by the parent class. In vctrs, double dispatch is implemented in such a that you need to call the methods for the parent class manually. For `vec_ptype2()` this means you need to call `df_ptype2()` (for data frame subclass) or `tib_ptype2()` (for tibble subclass). Similarly, `df_cast()` and `tib_cast()` are the workhorses for `vec_cast()` methods of subtypes of `data.frame` and `tbl_df`. These functions ensure take the union of the columns in `x` and `y`, and ensure shared columns have the same type.
