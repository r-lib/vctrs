...
```{r}
df3 <- tibble::new_tibble(df1, nrow = nrow(df1))

vec_rbind(df1, df3)
```

These fallbacks are not ideal but they make sense because all data frames share a common data structure. This is not generally the case for vectors, for example factors and characters have different representations. The downside is that the behaviour of R scripts will change when you implement vctrs methods for your subclass. This is why the proper coercion behaviour for your data frame class should be specified as soon as possible.
