...
The coercion system in vctrs is designed to make combination of multiple inputs consistent and extensible. Combinations occur in many places, such as row-binding, joins, subset-assignment, or grouped summaries functions that use the split-apply-combine strategy. For example:
...
* If the types of their inputs are compatible, indicate which of one is the richer type by returning it. If the types are of equal resolution, return any of the two.
...
The second generic, `vec_cast()`, is the one that look at the data and actually performs the conversion. Because it has access to more information than `vec_ptype2()`, it may be stricter and cause an error in more cases. `vec_cast()` has three possible behaviours:
