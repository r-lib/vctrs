...
This is a practical example of implementation of `vec_ptype2()` and `vec_cast()` methods. Related topics:
...
We'll illustrate how to implement coercion methods with a simple class that represents natural numbers. This is an existing class that features a constructor, methods for `print()` and subset.
...
Implementing double dispatch methods is very similar to implementing regular S3 methods. A typical signature for a `vec_ptype2()` methods looks like this. In these examples we are using roxygen2 tags to register the methods, but you can also register the methods manually in your NAMESPACE file or lazily with `s3_register()`.


### The self-to-self method
...
`vec_ptype2()` implements fallbacks to try and be compatible with simple classes, so it may seem that you don't need to implement the self-to-self coercion methods. However, you still have to implement this method implictly because this is how vctrs knows that it is dealing with a class that implements vctrs methods. Also, this is a bit more efficient.
...
In the following example, we implement a self-to-self ptype2 method for a hypothetical subclass of `<factor>` that has stricter combination semantics. The method throws when the levels of the two factors are not compatible.
...
Corresponding `vec_cast()` methods must be implemented for all `vec_ptype2()` methods. The self-to-self method is easy in this case, it just returns the target input:
...
The general pattern for `vec_cast()` methods is to convert the argument `x` to the type of `to`. The `vec_cast()` method should validate the values in `x` and make sure they conform to the values of `to`. We perform input validation in the `new_natural()` constructor, so that's a good fit for our `vec_cast()` implementations.

Note that for historical reasons, the order of the classes in the method name is in reverse order of the arguments in the function signature. The first class represents `to`, whereas the second class represents `x`.
