...
roxy_local_registration("base::print", "my_natural")
roxy_local_registration("base::[", "my_natural")
```
...
```{r, include = FALSE}
# Necessary because includeRmd() evaluated in a child of global
roxy_local_registration("vctrs::vec_ptype2", "my_natural.my_natural")
```

`vec_ptype2()` implements fallbacks to try and be compatible with simple classes, so it may seem that you don't need to implement the self-self coercion method. However, you still have to implement it explicitly because this is how vctrs knows that a class that is implementing vctrs methods (for instance this disable fallbacks to `base::c()`). Also, it makes your class a bit more efficient.
...
```{r, include = FALSE}
# Necessary because includeRmd() evaluated in a child of global
roxy_local_registration("vctrs::vec_ptype2", "my_natural.logical")
roxy_local_registration("vctrs::vec_ptype2", "my_natural.integer")
roxy_local_registration("vctrs::vec_ptype2", "integer.my_natural")
roxy_local_registration("vctrs::vec_ptype2", "logical.my_natural")
```
...
```{r, include = FALSE}
# Necessary because includeRmd() evaluated in a child of global
roxy_local_registration("vctrs::vec_ptype2", "my_natural.double")
roxy_local_registration("vctrs::vec_ptype2", "double.my_natural")
```
...
Corresponding `vec_cast()` methods must be implemented for all `vec_ptype2()` methods. The self-self method is easy in this case, it just returns the target input:
...
```{r, include = FALSE}
# Necessary because includeRmd() evaluated in a child of global
roxy_local_registration("vctrs::vec_cast", "my_natural.my_natural")
roxy_local_registration("vctrs::vec_cast", "my_natural.logical")
roxy_local_registration("vctrs::vec_cast", "my_natural.integer")
roxy_local_registration("vctrs::vec_cast", "my_natural.double")
```
...
```{r, include = FALSE}
# Necessary because includeRmd() evaluated in a child of global
roxy_local_registration("vctrs::vec_cast", "logical.my_natural")
roxy_local_registration("vctrs::vec_cast", "integer.my_natural")
roxy_local_registration("vctrs::vec_cast", "double.my_natural")
```
