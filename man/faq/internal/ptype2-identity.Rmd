...
`vec_ptype2()` returns the _promotion_ type of two vectors, if any. This is typically the richer type of the two so that automatic coercions are never lossy. For example, the promotion type of integer and double is the latter because double covers a larger range of values than integer.

`vec_ptype2()` is a [monoid](https://en.wikipedia.org/wiki/Monoid) over vectors, which in practical terms means that it is a well behaved operation for [reduction](https://purrr.tidyverse.org/reference/reduce.html). As such it needs an identity element, i.e. a value that doesn't change the result of the reduction. vctrs has two such identity values, `NULL` and __unspecified__ vectors.
