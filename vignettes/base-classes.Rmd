...
Then we add a `vec_print_footer` method to distinguish between ordered and unordered factors:

```{r}
vec_print_footer.ordrd <- function(x, ...) {
  cat("Levels: ", paste0(attr(x, "levels"), collapse = " > "), "\n", sep = "")
}
x
```
...
A date is a double. Most of the complexity is in printing and parsing; I'll just shim over the base Date methods 
...
format.date <- function(x) {
  format(vctrs::new_date(vec_data(x)), "%Y-%m-%d")
}
