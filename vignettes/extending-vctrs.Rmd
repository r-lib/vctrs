...
`vec_cast()` defines the possible set user-requested coercions, or casts. Because these are user initiated, the set of possible casts is much larger than the set of possible coercions.

```{r, echo = FALSE}
knitr::include_graphics("../man/figures/cast.png")
```

Like `vec_type2()`, `vec_cast()` uses double dispatch.

## Namespaces

The namespace declarations are a little tricky because of the dual role of the function: it needs to be registered as an S3 method, and exported as  function. This problem wasn't thought of during the creation of roxygen2 so the syntax is a little clunky:

```{r}
#' @export
#' @method vec_cast foofy
#' @export vec_cast.foofy
vec_cast.foofy <- function(x, y) {
  UseMethod("vec_cast.foofy")
} 
```

You also need to need to register all of the double-dispatch methods. Unfortuantely roxygen's heuristic for determing the name of the generic from the name of the method is suboptimal, so you'll need to manaully specify:

```{r}
vec_cast.foofy.integer <- function(x, y) {
  attr(x )
}
```
