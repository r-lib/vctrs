...
Both size and type are motivated by the utility of thinking about a function's interface, rather than it's implementation. See `vignette("stability")` for more details.
...
`vec_type()` is motivated by the need to capture the metadata associated with a vector, without capturing any data. Unfortunately, the `class()` of an object is inadequate for this purpose:
...
Instead vctrs takes advantage of R's vectorised nature and uses a __prototype__, a 0-observation slice of the vector. This is a miniature version of the vector that contains all of the attributes but none of the data. 

Conveniently, you can create many prototypes using existing base functions (e.g, `double()`, `factor(levels = c("a", "b"))`). vctrs provides a few helpers (e.g. `new_date()`, `new_datetime()`, `new_duration()`) where base R doesn't provide a convenient interface.

### Base prototypes

`vec_type()` creates a prototype from an existing object. However, many base vectors have uninformative printing methods for 0-length subsets, so while `vec_type()` is useful for programming with, it's not so great for interactive exploration. To resolve this problem vctrs also provides `vec_ptype()`, which is called for its side-effect of printing the prototype in a friendly way.

Using `vec_ptype()` allows us to explore the prototypes of classes in base R.
...
*  The prototype of a factor includes it levels. Levels are a character vector
   which can be arbitrarily long so the prototype just shows a hash; if the 
   hash of two factors is equal it's higly likely that their levels are also 
   equal.
...
*   Looking at the date/time classes, dates don't have any attributes, but 
    date-times (`POSIXct`) have a time zone and durations (`difftime`) have 
    units.
