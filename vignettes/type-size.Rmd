...
* `vec_type_common(x, NULL) == vec_ptype(x)`.
...
Like with `vec_ptype()`, the easiest way to explore `vec_type_common()` is with `vec_ptype_show()`: when given multiple inputs, it will print their common prototype. (In other words: program with `vec_type_common()` but play with `vec_ptype_show()`.)
