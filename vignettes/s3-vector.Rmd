...
```{r}
vec_ptype2.MYCLASS <- function(x, y, ...) UseMethod("vec_ptype2.MYCLASS")
...
Note that double dispatch is implemented using a special mechanism. In practice, this means:

- You don't need to implement default methods (they would never be called if you do).

- You can't call `NextMethod()`.

- The left methods of the form `vec_ptype2.leftclass()` (also called generic-methods) are purely indicative. They are never called. Only the right methods of the form `vec_ptype2.leftclass.rightclass()` are called.
...
```{r}
vec_ptype2.vctrs_percent <- function(x, y, ...) UseMethod("vec_ptype2.vctrs_percent")
```
...
* With package-level docs in `R/pizza-doc.R`. You can use
  `usethis::use_package_doc()` to initiate this package-level documentation.
  
* In `R/percent.R`. This makes the most sense when the vctrs S3 class is a
  modest and self-contained part of the overall package.
...
Next we implement methods for `vec_ptype2()` and `vec_cast()`, along with all the roxygen2 boilerplate needed to make them work. `vec_ptype2()` and `vec_cast()` have exactly the same roxygen2 considerations, so we'll explain everything via `vec_ptype2()` and just show the result for `vec_cast()`.
...
vec_ptype2.pizza_percent <- function(x, y, ...) {
  UseMethod("vec_ptype2.pizza_percent")
}
```
...
* Use `testthat::verify_output()` to test your format method.
  Customised printing is often a primary motivation for creating your own S3
  class in the first place, so this will alert you to unexpected changes in your
  printed output. Read more about `verify_output()` in the
  [testthat v2.3.0 blog post](https://www.tidyverse.org/blog/2019/11/testthat-2-3-0/); it's an
...
* Check for method symmetry; use `expect_s3_class()`, probably with 
  `exact = TRUE`, to ensure that `vec_c(x, y)` and `vec_c(y, x)` return the same
  type of output for the important `x`s and `y`s in your domain.
