...
```{r}
as.data.frame.percent <- function(x, row.names = NULL, optional = FALSE, ..., nm = paste(deparse(substitute(x), width.cutoff = 500L), collapse = " ")) {
  force(nm)
...
format.percent <- function(x, ...) {
  paste0(format.default(signif(x * 100, 3)), "%")
}
df
...
# Not quite right, but a reasonable place to start
...
`[<-.percent` <- function(x, i, value) {
  ...
  if (any(value < 0 | value > 1, na.rm = TRUE)) {
    stop("Percentages must be values between 0 and 1", call. = FALSE)
  }
  NextMethod()
}
