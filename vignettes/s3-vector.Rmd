...
* Subsetting (`[`, `[[`, `$`), `length<-`, and `rep()` methods automatically
  preserve attributes because they're built on top of `vec_recast()`. A 
  default `vec_recast()` works for all classes where the attributes are 
...
The vctr base class provides implementations of many important generics so that you don't have to. This means that most functions will just work out of the box without any additional effort on your part:
...
vec_cast.vctrs_percent.NULL    <- function(x, to) x
vec_cast.NULL.vctrs_percent    <- function(x, to) x
```
```{r, include = FALSE}
...
```{r}
vec_cast.vctrs_percent.integer <- function(x, to) percent(x)
vec_cast.integer.vctrs_percent <- function(x, to) vec_cast(vec_data(x), integer())
...
vec_type2.vctrs_percent.NULL    <- function(x, y) percent()
vec_type2.NULL.vctrs_percent    <- function(x, y) percent()
vec_type2.vctrs_percent.vctrs_percent <- function(x, y) percent()
```
```{r, include = FALSE}
...
vec_type2.vctrs_decimal.NULL    <- function(x, y) new_decimal()
vec_type2.NULL.vctrs_decimal    <- function(x, y) new_decimal()
...
vec_cast.vctrs_decimal.NULL     <- function(x, to) x
vec_cast.NULL.vctrs_decimal     <- function(x, to) x
```
...
To fix this, you need to provide a `vec_recast()` method. Note that this method dispatches on the `to` argument.

```{r}
vec_recast.vctrs_cached_sum <- function(x, to) {
  new_cached_sum(x, sum(x))
}
...
This works because most of the vctrs methods dispatch to the underlying base function, by first stripping off extra attributes with `vec_data()` and then reapplying them again with `vec_recast()`. The default `vec_recast()` method copies over all attributes, which is not appropriate here.

Note that `vec_recast.class` is subtly different from `vec_cast.class.class()`. `vec_recast()` is used when restoring attributes that have been lost; `vec_cast()` is used for coercions. This is easier to understand with a concrete example. Imagine factors were implements with `new_vectr()`. `vec_recast.factor()` would restore attributes back to an integer vector, but you would not want to allow manually casting an integer to a factor with `vec_cast()`. 
