...
If you are creating a new record class through `new_rcrd()` and implement at least one of these proxy methods, then you will likely need to implement them all due to the fact that there are `vctrs_rcrd` S3 methods for all of the proxy functions, which will intercept the typical fallthrough default behavior.
...
We can use the same proxy to fix `sort()`:
...
Typically, `vec_proxy_order()` would fallthrough to `vec_proxy_compare()`, but because record vectors define a `vec_proxy_order.vctrs_rcrd()` method, we have to define our own to ensure that our method isn't overriden by the record method.
