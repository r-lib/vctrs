...
## Appendix: Implementing a vctrs S3 class in a package
...
* Create documentation around your methods, for the sake of your user and to
  satisfy `R CMD check`.
...
We begin by adding `Import: vctrs` to our `DESCRIPTION` (with `usethis::use_package("vctrs")`) and then make all vctrs functions available to us by adding `#' @import vctrs` to `R/pizza-doc.R` (created with `usethis::use_package_doc()`). Usually, it's not good practice to `@import` a specific package but vctrs is deliberately designed with this use case in mind.

Then we make a place to dump some internal documentation that's needed to avoid R CMD check complaints. We don't expect any human will ever read this documentation.
...
We'll also add a call to `setOldClass()` for compatibility with S4:
...
It's a good idea to provide a function that tests if an object is of this class. If you do so, I'd document with `percent()`:
...
You'll also need to update the `percent()` document to reflect that `x` now means two different things:
...
Next we provide the key methods to make printing work. These are S3 methods so they don't need to be documented, but since `vec_ptype_abbr()` is a non-base generic, we need to import it:
...
### Double dispatch methods for coercion and casting

Next we the `vec_ptype2()` and `vec_cast()` methods along with all the roxygen2 boilerplate needed to make them work. `vec_ptype()` and `vec_cast()` have exactly the same structure, so we'll talk through `vec_ptype()` and then show what you'll need for `vec_cast()`.

The greatest boilerplate is needed fo `vec_ptype2.pizza_percent()` because it is both a method and a generic (thanks to the double-dispatch trick):

* As an S3 method it implements `vctrs::vec_ptype2()` for the `vctrs_percent`
  class.

* As a generic, `vec_ptype2.vctrs_percent()` has `default`, `vctrs_percent`, 
  and `double` methods (see below).
...
This creative take on the S3 system wasn't considered in the design of roxygen2, so we have to spell all of this out explicitly. `vec_ptype2.pizza_percent()` is a generic, so `R CMD check` will complain if it is not documented, although this documentation is not relevant for users. This is where we use the dummy internal help topic defined earlier.

Run `devtools::document()` to re-generate the `NAMESPACE`, and then take a quick look to familiarise yourself with what is generated. You should see this function appearing in two places (if you've never looked at `NAMESPACE` before note that roxygen2 puts it in alphabetical order):
...
Now we finish the work around `vec_ptype2()` by providing all needed methods. Each method has both a `@method` and an `@export` directive. These are purely methods so don't need to be documented in an `.Rd` file.
...
The exact same treatment applies to `vec_cast()`, which yields:
...
For simple casting and coercion functions, this is the formatting I'd use --- if everything fits on one line there's no need to use `{}` or add blank lines between method definitions. I think having all the methods and their documentation touching makes it more clear that they all belong together.

### Testing

It's good practice to test your new class. As well as the standard best practices for testing, we recommend that you:

* Use `devtools::test_file_coverage()` to ensure that every line of this 
  file is tested.

* Pay extra attention to zero-length inputs and missing values.

* Use `testthat::verify_output()` to add some 
  [golden tests](https://ro-che.info/articles/2017-12-04-golden-tests) for 
  your format method.

* It's good practice to check method symmetry; use `expect_s3_class()` to 
  ensure that both `vec_c(x, y)` and `vec_c(y, x)` return the same type of 
  output for the important `x`s and `y`s in your domain.
  
* Use `expect_error()` to check that inputs that can't be combined fail with an
  error. Here, you should be generally checking for a specific error `class`,
  not an error message. Useful classes include `vctrs_error_assert_ptype`, 
...
If your test succeeds when run by `devtools::test()` but fail when run in R CMD check, it is almost certainly due to a S3 method registration problem; i.e. there's something wrong with your `NAMESPACE`.
