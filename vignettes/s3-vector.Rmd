...
This class is intended to live in the pizza package, see the ["Implementing"](#implementing-a-vctrs-s3-class-in-a-package) section below for details. The S3 class is therefore prefixed by `pizza_`.
...
s3_register("base::format", "pizza_percent")
```
...
However this only works if the attributes for both objects are the same. Also the default methods are a bit slower. It is always a good idea to provide an explicit coercion method for the case of identical classes. So we'll start by saying that a `pizza_percent` combined with a `pizza_percent` yields a `pizza_percent`, which we indicate by returning a prototype generated by the constructor.

```{r}
vec_ptype2.pizza_percent.pizza_percent <- function(x, y, ...) new_percent()
```
...
If we implemented `vec_ptype2.pizza_percent.numeric()`, it would never be called.

```{r}
vec_ptype2.pizza_percent.double <- function(x, y, ...) double()
vec_ptype2.double.pizza_percent <- function(x, y, ...) double()
```
...
Implementing `vec_cast.pizza_percent.numeric()` has no effect.
...
Again, as a convenience, consider providing an `as_percent()` function that makes use of the casts defined in your `vec_cast.pizza_percent()` methods:
...
new_decimal <- function(x = double(), digits = 2L) {
  ...
  new_vctr(x, digits = digits, class = "pizza_decimal")
}
...
format.pizza_decimal <- function(x, ...) {
  sprintf(paste0("%-0.", digits(x), "f"), x)
}

vec_ptype_abbr.pizza_decimal <- function(x, ...) {
  "dec"
}
...
```{r}
vec_ptype_full.pizza_decimal <- function(x, ...) {
...
```{r}
vec_ptype2.pizza_decimal.pizza_decimal <- function(x, y, ...) {
  new_decimal(digits = max(digits(x), digits(y)))
}
vec_cast.pizza_decimal.pizza_decimal <- function(x, to, ...) {
...
```{r}
vec_ptype2.pizza_decimal.double <- function(x, y, ...) x
vec_ptype2.double.pizza_decimal <- function(x, y, ...) y

vec_cast.pizza_decimal.double  <- function(x, to, ...) new_decimal(x, digits = digits(to))
vec_cast.double.pizza_decimal  <- function(x, to, ...) vec_data(x)
...
new_cached_sum <- function(x = double(), sum = 0L) {
  ...
  new_vctr(x, sum = sum, class = "pizza_cached_sum")
}
...
```{r}
obj_print_footer.pizza_cached_sum <- function(x, ...) {
...
```{r}
vec_math.pizza_cached_sum <- function(.fn, .x, ...) {
  cat("Using cache\n")
  switch(.fn,
    sum = attr(.x, "sum"),
...
```{r}
vec_restore.pizza_cached_sum <- function(x, to, ..., i = NULL) {
  new_cached_sum(x, sum(x))
}
...
new_rational <- function(n = integer(), d = integer()) {
  ...
  new_rcrd(list(n = n, d = d), class = "pizza_rational")
}
```
...
```{r}
format.pizza_rational <- function(x, ...) {
  n <- field(x, "n")
  d <- field(x, "d")
...
vec_ptype_abbr.pizza_rational <- function(x, ...) "rtnl"
vec_ptype_full.pizza_rational <- function(x, ...) "rational"
...
```{r}
vec_ptype2.pizza_rational.pizza_rational <- function(x, y, ...) new_rational()
vec_ptype2.pizza_rational.integer <- function(x, y, ...) new_rational()
vec_ptype2.integer.pizza_rational <- function(x, y, ...) new_rational()

vec_cast.pizza_rational.pizza_rational <- function(x, to, ...) x
vec_cast.double.pizza_rational <- function(x, to, ...) field(x, "n") / field(x, "d")
vec_cast.pizza_rational.integer <- function(x, to, ...) rational(x, 1)
...
new_decimal2 <- function(l, r, scale = 2L) {
  ...
  new_rcrd(list(l = l, r = r), scale = scale, class = "pizza_decimal2")
}
...
format.pizza_decimal2 <- function(x, ...) {
...
vec_proxy_equal.pizza_rational <- function(x, ...) {
  n <- field(x, "n")
  d <- field(x, "d")
  gcd <- gcd(n, d)
...
```{r}
vec_proxy_compare.pizza_rational <- function(x, ...) {
  field(x, "n") / field(x, "d")
}
...
```{r}
new_poly <- function(x) {
  new_list_of(x, ptype = integer(), class = "pizza_poly")
}
...
vec_ptype_full.pizza_poly <- function(x, ...) "polynomial"
vec_ptype_abbr.pizza_poly <- function(x, ...) "poly"

format.pizza_poly <- function(x, ...) {
...
obj_print_data.pizza_poly <- function(x, ...) {
  if (length(x) == 0)
    return()
...
The resulting objects will inherit from the `pizza_list_of` class, which provides tailored methods for `$`, `[[`, the corresponding assignment operators, and other methods.
...
```{r}
vec_proxy_compare.pizza_poly <- function(x, ...) {
  x_raw <- vec_data(x)
...
new_meter <- function(x) {
  ...
  new_vctr(x, class = "pizza_meter")
}

format.pizza_meter <- function(x, ...) {
...
```{r}
vec_arith.pizza_meter <- function(op, x, y, ...) {
  UseMethod("vec_arith.pizza_meter", y)
}
vec_arith.pizza_meter.default <- function(op, x, y, ...) {
  stop_incompatible_op(op, x, y)
}
```
...
```{r, error = TRUE}
vec_arith.pizza_meter.pizza_meter <- function(op, x, y, ...) {
  switch(
    op,
    "+" = ,
...
```{r, error = TRUE}
vec_arith.pizza_meter.numeric <- function(op, x, y, ...) {
  switch(
    op,
    "/" = ,
    "*" = new_meter(vec_arith_base(op, x, y)),
...
vec_arith.numeric.pizza_meter <- function(op, x, y, ...) {
  switch(
    op,
    "*" = new_meter(vec_arith_base(op, x, y)),
    stop_incompatible_op(op, x, y)
...
For completeness, we also need `vec_arith.pizza_meter.MISSING` for the unary `+` and `-` operators:

```{r}
vec_arith.pizza_meter.MISSING <- function(op, x, y, ...) {
  switch(op,
    `-` = x * -1,
    `+` = x,
    stop_incompatible_op(op, x, y)
