...
vctrs provides four "proxy" generics. Three of these let you control how your class determines equality and ordering:
...
*   `vec_proxy_compare()` specifies how to compare the elements of your vector.  This proxy is used in `<`, `<=`, `>=`, `>`, `min()`, `max()`, `median()`, and `quantile()`. Its default implementation calls `vec_proxy_order()` and you normally don't need to implement this proxy.
...
By default, `vec_proxy_equal()`, `vec_proxy_compare()`, and `vec_proxy_order()` just call `vec_proxy()`.
...
By default, list ordering works by first occurrence:
...
For an alternative ordering by the coefficients, we define a `vec_proxy_order()` method:
...
However, we still can't compare individual elements because by default lists are not comparable:

```{r, error = TRUE}
p < p[2]
```

To enable comparison, we implement `vec_proxy_compare()` and forward to `vec_proxy_order()`:
