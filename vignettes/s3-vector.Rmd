...
Defining S3 methods interactively is fine for iteration and exploration, but if your vector lives in a package, you need to do a few more things:
...
### User-friendly constructor and friends

Do we refer to it as `percent` or `vctrs_percent`?

Flesh out the roxygen comments. Mine code above for more examples.

Export & rdname the "friends".
...
#' @param x Double
#'
#' @return An S3 vector of class `vctrs_percent`.
#' @export
#' @examples
#' percent()
percent <- function(x = double()) {
...
TO ADD: some specifics about what this does in NAMESPACE. So at least one specific mention of e.g. `S3method(vec_ptype2,vctrs_percent)`.

Here is the first bit of boilerplate necessary to express which implicit coercions are OK for `percent`, which is controlled by `vctrs::vec_ptype2()`.

```{r eval = FALSE}
#' @method vec_ptype2 vctrs_percent               # register as method
#' @export vec_ptype2.vctrs_percent               # export as generic
#' @export                                        # export as method
#' @rdname percent-vctrs                          # document (internally)
vec_ptype2.vctrs_percent <- function(x, y, ...) {
  UseMethod("vec_ptype2.vctrs_percent", y) 
}
```

The namespace declarations are a little tricky because `vec_ptype2.vctrs_percent()` is simultaneously two things:

  * An S3 method: it implements `vctrs::vec_ptype2()` for the `vctrs_percent`
    class.
  * A generic: thanks to the double-dispatch trick, `vec_ptype2.vctrs_percent()`
    is a generic with `default`, `vctrs_percent`, and `double` methods (see
    below).

This creative take on the S3 system wasn't considered in the design of roxygen2, so we have to spell all of this out explicitly. Because `vec_ptype2.vctrs_percent()` presents as an exported generic, `R CMD check` will complain if it is not documented, although this documentation is not relevant for users. This is where we use the dummy internal help topic defined earlier.

Now we finish the work around `vec_ptype2()`. Each method has both a `@method` and an `@export` directive.
...
The exact same treatment applies to `vec_cast()`. Leave out because it's redundant? Or include to be complete?
...
Say something about tests?

https://github.com/tidyverse/googlesheets4/blob/CellData-and-formulas/tests/testthat/test-formula.R

https://github.com/tidyverse/glue/blob/master/tests/testthat/test-vctrs.R
