...
* `vec_proxy_compare()` specifies how to compare the elements of your vector. It is identical to `vec_proxy_order()`, with the exception of lists, which provide ordering (by first occurrence) but not elementwise comparison. This proxy is used in `<`, `<=`, `>=`, `>`, `min()`, `max()`, `median()`, and `quantile()`.
