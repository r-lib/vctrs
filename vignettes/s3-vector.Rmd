...
First, the pizza package needs to include vctrs in the `Imports` section of its `DESCRIPTION` (perhaps by calling`usethis::use_package("vctrs")`. Then we make all vctrs functions available within the pizza package by including the directive `#' @import vctrs` somewhere. You can use `usethis::use_package_doc()` to create package-level documentation in `R/pizza-doc.R`, in which case that is also the logical place to put `#' @import vctrs`. Usually, it's not good practice to `@import` the entire namespace of a package, but vctrs is deliberately designed with this use case in mind.

Remember to call `devtools::document()` regularly, as you develop, to regenerate `NAMESPACE` and the `.Rd` files.

Next we make a place to dump some internal documentation that's needed to avoid `R CMD check` complaints. We don't expect any human to ever read this documentation. This, and all subsequent code, presumably lives in `R/percent.R`:
...
Next we implement methods for `vec_ptype2()` and `vec_cast()`, along with all the roxygen2 boilerplate needed to make them work. `vec_ptype2()` and `vec_cast()` have exactly the same structure, so we'll explain everything via `vec_ptype2()` and just show the result for `vec_cast()`.

The greatest boilerplate is needed for `vec_ptype2.pizza_percent()` because it is both a method and a generic (thanks to the double-dispatch trick):
...
(The marginal comments are just for exposition and can be omitted in your own packages.)

This creative take on the S3 system wasn't considered in the design of roxygen2, so we have to spell all of this out explicitly. Since `vec_ptype2.pizza_percent()` is a generic, `R CMD check` will complain if it is not documented, although this documentation is not relevant for users. This is the purpose of the dummy internal help topic defined earlier.

Remember to run `devtools::document()` to re-generate the `NAMESPACE`, and then take a quick look to familiarise yourself with what is generated. You should see this function appearing in two places (if you've never looked at `NAMESPACE` before note that roxygen2 puts it in alphabetical order):
...
Now we finish the work around `vec_ptype2()` by providing all needed methods. Each method has both a `@method` and an `@export` directive. These are purely methods so they don't need to be documented in an `.Rd` file.
...
For simple casting and coercion functions, this is the recommended formatting. If everything fits on one line, there's no need to use `{}` or add blank lines between method definitions. This compact formatting reinforces the common purpose of these methods and their documentation.
...
* Use `testthat::verify_output()` to add some 
  ...
  your format method. Customised printing is often a primary motivation for
  creating your own S3 class in the first place, so it makes sense to actively
  monitor this.

* Check for method symmetry; use `expect_s3_class()` to 
  ensure that `vec_c(x, y)` and `vec_c(y, x)` return the same type of output for
  the important `x`s and `y`s in your domain.
  
* Use `testthat::expect_error()` to check that inputs that can't be combined
  fail with an error. Here, you should be generally checking the class of the
  error, not its message. Relevant classes include `vctrs_error_assert_ptype`,
