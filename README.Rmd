...
Internally, vctrs represents the class of a vector with a 0-length subset of the vector. This captures all the attributes of the class, and in many cases you can use existing base functions like (e.g, `double()`, `factor(levels = c("a", "b"))`). You can use `vec_ptype()` to get a textual representation of prototype of an object:
